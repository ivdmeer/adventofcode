package nl.imm.adventofcode.year2022.day5.part2;
...
class SupplyStacksTest {
	...
	void exerciseTest() {
		...
		// then
		assertEquals("TCGLQSLPW", topCrates);
	}
	...
	private String rearrangeCrates(List<String> rearrangements, Map<Integer, Stack<String>> crateStackMap) {
		for (String move : rearrangements) {
			...
			// move crates from stack to tempStack
			for (int count = 0; count < amountOfCrates; count++) {
				String crate = crateStackMap.get(indexSrc).pop();
				tempStack.push(crate);
			}

			// move crates from tempStack in reverse order to restore original stacking order, to destination stack.
			ListIterator<String> platesListIterator = tempStack.listIterator(tempStack.size());
			while (platesListIterator.hasPrevious()) {
				String crate = platesListIterator.previous();
